...
- **System specs must select elements via `data_test(...)`** (`spec/support/helpers/test_element_helper.rb`) **or visible text — never a CSS class or id.** Classes can change with styling and silently break tests; add `data-testid` attributes to views as needed. `Capybara.enable_aria_label` is `false` in this project, so `click_on`/`click_button` can't match an icon-only button by its `aria-label` alone — give it a `data-testid` instead.
...
- **Avoid multi-level chained safe-navigation** (`turn&.errors&.[](:base)&.first`). Prefer an explicit private helper that returns an empty/blank result when the receiver is nil (e.g. a `turn_base_errors` method returning `[]`), then call plain methods on its result.
- **Interactive-state markup (`disabled`/`hidden` attributes reflecting live game state) must be rendered from presenter/model state server-side**, not left for JS to correct after page load — a Stimulus controller may still layer client-only refinements on top (e.g. a checked-checkbox count), but the initial server-rendered HTML must already be correct without JS running.
...
- **CSS nesting**: elements are written as the full class name (`.block__element { ... }`) nested inside `.block { ... }`, not `&__element` shorthand. Modifiers use `&.block--modifier`, and pseudo-classes/states use `&:hover`/`&.is-open` (see `simple-page.css`, `playing-card.css`).
- **Overriding a vendor Optics rule written as nested CSS** (e.g. `.op-page { .op-page__main {...} }` in `optics-overrides/`) must be nested the same way in your override — a flat, un-nested selector has lower specificity than the vendor's nested one and silently loses, regardless of which stylesheet loads later.
