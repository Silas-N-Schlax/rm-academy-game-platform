...
- `app/views/games/show.html.slim` subscribes with `turbo_stream_from @game` and re-renders via `render @game`, which uses Rails' polymorphic partial lookup — `GoFishGame` renders `go_fish_games/_go_fish_game`, `CrazyEightsGame` renders `crazy_eights_games/_crazy_eights_game`. `show.html.slim` builds one `@game.presenter_class.new(@game, current_user, turn:, turn_timer_seconds:)` and passes it down as the sole `presenter:` local — `Game#presenter_class` raises `NotImplementedError` by default, mirroring `engine_class`/`turn_class`. Rummy implements a real presenter (`RummyPresenter`, `app/presenters/rummy_presenter.rb`) that the shared `_game_board*` partials call directly (`presenter.hand`, `presenter.opponents`, etc.) instead of reaching into a hash. Go Fish and Crazy Eights aren't converted yet — they still build their own display data directly from `implementation` inline in their own partials, and `presenter_class` returns a `NullPresenter` stub (`app/presenters/null_presenter.rb`) purely so the shared show view doesn't blow up on them. A `GamePresenter` base class abstracted from `RummyPresenter`'s shape is deferred until Go Fish's conversion gives a second real data point.
