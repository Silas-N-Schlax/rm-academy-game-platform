...
- Adding or editing a Stimulus controller and running `bin/rails stimulus:manifest:update` only fixes `index.js` — the actual browser-served bundle at `app/assets/builds/application.js` is a separate esbuild output and stays stale until `yarn build` runs (or `bin/dev`'s `yarn build --watch` is already running and picks up the change). Running system specs without the watcher active means JS changes silently don't take effect: markup and `data-action` wiring all look correct, the controller connects, but the new behavior just doesn't run. This looks identical to a Stimulus wiring bug and cost real debugging time (2026-07-28, leaderboard filter drawer) before the stale bundle was found to be the actual cause.
- Top-level helper methods defined at the bottom of a spec file (e.g. `def create_finished_game(...)`) are **not scoped to that file** — RSpec loads every spec file into the same global namespace, so two files defining a same-named helper with different signatures silently clash (whichever loads last wins). This passes when either spec file runs alone and only breaks when the full suite runs together, which makes it confusing to track down. Give file-local top-level spec helpers unique names across the suite, or move a genuinely shared one into `spec/support/`.
